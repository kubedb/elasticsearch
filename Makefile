...
DOCKERFILE_PROD  = hack/docker/es-operator/in.Dockerfile
DOCKERFILE_DBG   = hack/docker/es-operator/dbg.Dockerfile
...
check-license:
	...
	@docker run --rm 	                                 \
		...
		--env HTTPS_PROXY=$(HTTPS_PROXY)                 \
		$(BUILD_IMAGE)                                   \
		ltag -t "./hack/license" --excludes "vendor contrib third_party" -v
